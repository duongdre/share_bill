...
class PersonNotifier extends _$PersonNotifier {
  ...
  Future<List<Person>> fetchAllPerson() async {
    try {
      ...
      if (snapshot.exists && snapshot.value != null) {
        ...
        print('📊 Raw persons data: ${data.keys.length} items');

        data.forEach((key, value) {
          try {
            final person = Person.fromMap(value as Map<dynamic, dynamic>);
            print('✅ Successfully parsed person: ${person.name} (${person.uid})');
            allPerson.add(person);
            allPersonMapping[key] = value;
          } catch (e) {
            print('❌ Error parsing person with key $key: $e');
            print('❌ Person data: $value');
          }
        });
      } else {
      ...
      return List.from(allPerson);

    } catch (error) {
      print('❌ Error fetching persons data: $error');
      return [];
    }
  }
  ...
  Person? findPersonWithUid(String userId) {
    try {
      if (allPersonMapping.containsKey(userId)) {
        return Person.fromMap(allPersonMapping[userId] as Map<dynamic, dynamic>);
      }

      // Also search in the allPerson list as backup
      for (final person in allPerson) {
      ...
      print('❌ Error finding person with uid $userId: $e');
      return null;
    }
  }
